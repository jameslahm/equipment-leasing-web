...
import { AuthContext, getAllEquipments, deleteEquipment } from "../utils";
...
const headCells = [
  ...
  { id: "confirmed_back", th: false, disablePadding: false, label: "ConfirmedBack" },
  {
    id: "actions",
    label: "Actions",
    th: false,
...
function UserList() {
  ...
  const { data = {}, isLoading } = useQuery(queryKey, (key, options, token) =>
    getAllEquipments(options, token)
  );

  const [mutate] = useMutation(deleteEquipment);
